...
async def try_again_handler(client: Client, query: CallbackQuery):
    user_id = query.from_user.id
    channels = [AUTH_CHANNEL, AUTH_REQ_CHANNEL]
    not_joined = []
    join_links = []
    for ch in channels:
        try:
            member = await client.get_chat_member(ch, user_id)
            # If user is banned or kicked, treat as not joined
            if member.status in [enums.ChatMemberStatus.BANNED, enums.ChatMemberStatus.RESTRICTED]:
                not_joined.append(ch)
        except pyrogram.errors.UserNotParticipant:
            not_joined.append(ch)
        except Exception as e:
            # If channel is private and join request is needed, treat as not joined
            not_joined.append(ch)
    if not_joined:
            ...
            try:
                ...
                if chat.username:
                    ...
                    if hasattr(chat, 'invite_link') and chat.invite_link:
                        ...
                        # fallback: ask admin to set invite link
                        link = f"https://t.me/"  # fallback
                join_links.append((chat.title or str(ch), link))
            except Exception:
                join_links.append((str(ch), f"https://t.me/"))
        text = "<b>❗ You must join all required channels to use the bot.</b>\n"
        for title, link in join_links:
            text += f"\n➡️ <a href='{link}'>{title}</a>"
        text += "\n\nAfter joining, click Try Again."
        btn = [[InlineKeyboardButton("🔄 Try Again", callback_data="try_again")]]
        ...
        return
    # If user is member of both channels, proceed to next step
    await query.answer("✅ You are a member of all required channels!", show_alert=True)
    # You can add your next step logic here, e.g., show main menu or allow bot usage
    await query.message.edit_text(
        "<b>✅ You have joined all required channels. You can now use the bot!</b>",
        parse_mode=enums.ParseMode.HTML
    )
...
@Client.on_callback_query(filters.regex(r"^correct_spell#"))
async def correct_spell_cb(bot, query):
    """
    Handles the user's confirmation to search for the corrected spelling.
    """
    _, corrected_query = query.data.split("#")
    
    await query.answer("Searching for the corrected query...")
    
    # Get the original message to maintain context
    original_message = query.message.reply_to_message or query.message
    
    # Create a new message object with the corrected text
    mock_message = original_message
    mock_message.text = corrected_query

    # Remove the suggestion message to keep the chat clean
    await query.message.delete()
    
    # Trigger the auto_filter with the new query
    await auto_filter(bot, mock_message)
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if int(user) != 0 and query.from_user.id != int(user):
            return await query.answer(script.ALRT_TXT, show_alert=True)
        await query.answer("ᴛʜᴀɴᴋs ꜰᴏʀ ᴄʟᴏsᴇ ᴍᴇ !")
        await query.message.delete()
        try:
            await query.message.reply_to_message.delete()
        except:
    ...
    #     )
        ...
        terms_link = f"<a href='https://t.me/{bot_username}?start=terms'>ᴛ&ᴄ</a>"
        
        # This automatically gets the latest prices from your config
        formatted_text = script.FREE_TXT.format(
            query.from_user.mention,
            price_7_days=PRICING_POINTS.get(7),
            price_30_days=PRICING_POINTS.get(30),
            price_90_days=PRICING_POINTS.get(90),
            price_180_days=PRICING_POINTS.get(180),
            price_365_days=PRICING_POINTS.get(365),
    ...
    #     )
        ...
        await save_group_settings(grp_id, "template", script.IMDB_TEMPLATE_TXT)
        ...
        if not files:
            if settings["spell_check"]:
                ...
                if is_misspelled:
                    ...
                    btn = [[
                        InlineKeyboardButton(
                            f"✅ Search for '{is_misspelled}'",
                            callback_data=f"correct_spell#{is_misspelled}"
                        )],
                        [InlineKeyboardButton("❌ Cancel", callback_data="close_data")]
                    ]
                    await msg.reply_text(
                        f"I couldn't find anything for `{search}`.\nDid you mean: **{is_misspelled}**?",
                        reply_markup=InlineKeyboardMarkup(btn)
                    )
                else:
                    # Fallback if no good suggestion is found
