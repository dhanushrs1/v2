...
from utils_extra import is_check_admin
...
async def grp_cmds(client, message):
    ...
    if not user_id:
        return await message.reply(
            "<b>💔 You are an anonymous admin and do not have permission to use this command.</b>"
        )
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("🔒 Group-only command! Use this in a group to make it work. 👍")
    grp_id = message.chat.id
    if not await is_check_admin(client, grp_id, message.from_user.id):
        return await message.reply_text("<b>😕 Looks like you’re not an admin here. Ask an admin to help!</b>")
