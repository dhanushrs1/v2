...
from utils.invite_link import generate_invite_links
...
async def start(client: Client, message):
    ...
    if len(m.command) == 2 and m.command[1].startswith("notcopy"):
        try:
            ...
            print(f"Error in notcopy verification: {e}")
            await message.reply("<b>Error processing verification link</b>")
            return
    ...
    if len(message.command) == 1:  # Just /start without parameters
        buttons = [
            [
                InlineKeyboardButton(
    ...
    if len(message.command) == 2 and message.command[1].startswith("reff_"):
        ...
        if fromuse >= PREMIUM_POINT:
            ...
            await client.send_message(user_id, "🎉 Congratulations! You have received a premium subscription through referrals!")
        else:
    ...
    if len(message.command) == 2 and message.command[1].startswith("getfile"):
        try:
            ...
            await message.reply_text("Error processing search request")
            return
    ...
    if len(message.command) == 2 and message.command[1] == "ads":
        try:
            ...
            print(f"Error in ads: {e}")
            await message.reply_text("Error loading ads")
            return

    # Handle terms
    if len(message.command) == 2 and message.command[1] == "terms":
        buttons = [[
    ...
    if len(message.command) == 2:
        ...
        try:
            ...
            await message.reply_text("<b>⚠️ Invalid file link ⚠️</b>")
            return
        ...
        # Check user membership in required channels before showing force subscription
        async def is_member_of_all_required(client, user_id):
        ...
        user_is_member = await is_member_of_all_required(client, message.from_user.id)
        if not user_is_member:
            # Force subscription check
            def ensure_try_again(btn):
                # Always add Try Again button if not present
                has_try_again = any(
                    any(
                        (b.text and ("try again" in b.text.lower() or "ᴛʀʏ ᴀɢᴀɪɴ" in b.text.lower()))
                        for b in row
                    ) for row in btn
                )
                if not has_try_again:
                    btn.append([
                        InlineKeyboardButton(
                            "♻️ ᴛʀʏ ᴀɢᴀɪɴ ♻️",
            ...
            if settings.get("fsub_id", AUTH_CHANNEL) == AUTH_REQ_CHANNEL:
                if AUTH_REQ_CHANNEL:
                    try:
                        btn = await generate_invite_links(client)
                    except Exception as e:
                        print(f"Error creating invite link: {e}")
                        btn = []
                    btn = ensure_try_again(btn)
                ...
                channel = int(id) if id else AUTH_CHANNEL
                btn = []
                try:
                    btn.extend(await generate_invite_links(client))
                    ...
                    print(f"Error in force sub check: {e}")
        # ...existing code...

        # Check premium access and verification
        user_id = m.from_user.id
        ...
        if data and data.startswith("allfiles"):
            try:
                ...
                print(f"Error in allfiles: {e}")
                await message.reply_text("<b>⚠️ Error processing files ⚠️</b>")
                return
        ...
        try:
            ...
            if not files_:
                try:
                    ...
                    pass
                await message.reply("<b>⚠️ File not found ⚠️</b>")
                return
            ...
            await message.reply_text("<b>⚠️ Error processing file ⚠️</b>")
            return
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/hd_cinema_updates</code>"
        )
...
async def set_tutorial_2(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>ᴄᴏᴍᴍᴀɴᴅ ɪɴᴄᴏᴍᴘʟᴇᴛᴇ !!\n\nᴜꜱᴇ ʟɪᴋᴇ ᴛʜɪꜱ -</b>\n\n<code>/set_tutorial_2 https://t.me/hd_cinema_updates</code>"
        )
...
async def set_tutorial_3(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_tutorial https://t.me/hd_cinema_updates</code>"
        )
